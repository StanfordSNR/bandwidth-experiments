...
string AWSv4Sig::sha256_( const string& in )
{
  ...
  SHA256( reinterpret_cast<const unsigned char*>( in.c_str() ), in.length(), buf );
                             ...
                             const std::string& payload_hash )
{
  ...
  unsigned char buf[SHA256_DIGEST_LENGTH];
  vector<uint8_t> skey = derive_signing_key_( secret, request_date.substr( 0, 8 ), region, service );
  HMAC( EVP_sha256(),
        skey.data(),
        SHA256_DIGEST_LENGTH,
  ...
  req.str( "" );
  req << ALGORITHM_ << " Credential=" << akid << '/' << cred_scope << ", SignedHeaders=" << signed_headers
      << ", Signature=" << signature;
