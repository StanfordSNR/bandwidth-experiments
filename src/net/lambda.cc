...
namespace {
// Encodes the given string using the percent-encoding mechanism described in
// RFC 3986. Used for canonicalizing the path for an HTTP requests to AWS.
//
...
string uri_encode( const string& unencoded )
{
  ...
  for ( const char c : unencoded ) {
    if ( !isalnum( c ) && c != '-' && c != '_' && c != '.' && c != '~' && c != '/' ) {
      // Encode it
      encoded << '%' << uppercase << setw( 2 ) << static_cast<int>( static_cast<unsigned char>( c ) ) << nouppercase;
    } else {
      encoded << c;
    }
  }
...
string to_string( const InvocationType& invocation_type )
{
  switch ( invocation_type ) {
    ...
    default:
      throw runtime_error( "invalid invocation type" );
  }
}
...
string to_string( const LogType& log_type )
{
  switch ( log_type ) {
    ...
    default:
      throw runtime_error( "invalid log type" );
  }
}
